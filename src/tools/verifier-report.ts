...
export function createVerifierReportTool(orchestrator: Orchestrator): ToolDefinition {
  return tool({
    ...
    execute: async (args, context) => orchestrator.recordVerifierReport(await authorizeThenAsk(orchestrator, context, "goat_verifier_report"), args.findings, operationKey("goat_verifier_report", context, "", args.findings)),
  });
}
